    ...
    <div className="stack">
      <div className="card stack">
        <div className="row" style={{ justifyContent: "space-between" }}>
          ...
          <div className="row">
            <button className="btn" onClick={() => setShowEdit((s) => !s)}>Modifier contrat</button>
            <a className="btn primary" href={`/api/contracts/${contractId}/pdf?month=${month}`}>
              Télécharger PDF
            </a>
      ...
      <div className="card stack">
        ...
        <div className="grid cols-2">
          <div className="stack">
            ...
            <div className="row">
              <div className="field" style={{ width: 120 }}>
                <label>Repas</label>
                <input type="number" min={0} value={entryForm.mealsCount} onChange={(e) => setEntryForm({ ...entryForm, mealsCount: e.target.value })} />
              </div>
              <div className="field" style={{ flex: 1 }}>
                <label>Notes</label>
          ...
          <div className="stack">
            ...
            <label className="row"><input type="checkbox" checked={entryForm.isHoliday} onChange={(e) => setEntryForm({ ...entryForm, isHoliday: e.target.checked })} /> Jour férié</label>
            ...
            <div className="row">
              ...
              {entryMsg ? <span className="small">{entryMsg}</span> : null}
            </div>
          </div>
        </div>
      </div>
            ...
            <table>
              ...
              <tbody>
                {data.summary.dayRows.map((row: DayRow) => (
                  <tr key={row.date}>
                    <td>{row.date}</td>
                    <td>{row.entry?.startTime ?? "-"}</td>
                    <td>{row.entry?.endTime ?? "-"}</td>
                    <td>{row.hoursDone ? row.hoursDone.toFixed(2) : "-"}</td>
                    <td>{row.complementaryHours ? row.complementaryHours.toFixed(2) : "-"}</td>
                    <td>{row.entry?.mealsCount ?? "-"}</td>
                    <td className="small">
                      {[
                        row.entry?.isPlannedAbsence && "Abs. prévue",
                        row.entry?.isUnplannedAbsence && "Abs. non prévue",
                        row.entry?.isHoliday && "Férié",
                        row.entry?.isUnavailable && "Indispo",
                      ]
                        .filter(Boolean)
                        .join(", ") || "-"}
                    </td>
            ...
            <table>
              ...
              <tbody>
                {data.summary.weekly.details.map((w: WeeklyDetailRow) => (
                  <tr key={w.weekStart}>
                    <td>{w.weekStart}</td>
                    <td>{w.hoursDone.toFixed(2)} h</td>
                    <td>{w.overtimeHours.toFixed(2)} h</td>
                    <td>{formatCurrency(w.overtimeHours * effectiveRate * (data.contract.overtimeRatePercent / 100))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {tab === "mois" ? (
          <div className="stack">
            <div className="summary-grid">
              <div className="card">
                <div className="summary-row"><span>Heures prévues (lissées)</span><strong>{formatHours(rounded.expectedMonthlyHours)}</strong></div>
                ...
                <div className="summary-row"><span>Heures majorées (&gt;45h)</span><strong>{formatHours(rounded.overtimeHoursMonth)}</strong></div>
                <div className="summary-row"><span>Taux horaire effectif</span><strong>{effectiveRate.toFixed(2)} €/h</strong></div>
                <div className="summary-row"><span>Majoration (%)</span><strong>{data.contract.overtimeRatePercent}%</strong></div>
              </div>
              <div className="card">
                <div className="summary-row"><span>Brut base</span><strong>{formatCurrency(rounded.brutBase)}</strong></div>
                <div className="summary-row"><span>Brut complémentaires</span><strong>{formatCurrency(rounded.brutComplementary)}</strong></div>
                <div className="summary-row"><span>Brut majoration</span><strong>{formatCurrency(rounded.brutOvertime)}</strong></div>
                <div className="summary-row"><span>Prime précarité CDD</span><strong>{formatCurrency(rounded.primeMensuelle)}</strong></div>
                <div className="summary-row total"><span>Total brut</span><strong>{formatCurrency(rounded.totalBrut)}</strong></div>
                <div className="summary-row"><span>Net (x 0.7812)</span><strong>{formatCurrency(rounded.net)}</strong></div>
            ...
            <div className="card stack">
              ...
              {complementaryDays.length === 0 ? (
                <div className="muted small">Aucun jour avec heures complémentaires.</div>
              ) : (
                <div className="table-wrap">
                  <table>
                    <thead>
                    ...
                    <tbody>
                      {complementaryDays.map((d) => (
                        <tr key={d.date}>
                          <td>{d.date}</td>
                          <td>{d.hoursDone.toFixed(2)} h</td>
                          <td>{d.complementaryHours.toFixed(2)} h</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
